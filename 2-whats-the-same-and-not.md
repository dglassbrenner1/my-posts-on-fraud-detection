...
1. **Set up your training/validation framework**: Choose indices for either training data $T_1\subseteq\{1,\ldots,n\}$ and validation data $V_1\subseteq\{1,\ldots,n\}$, or a $k$-fold cross-validation setup $(T_1, V_1),\ldots, (T_k, V_k)$ for $k\geq 1$. (I'm identifying members $(\mathbf{X}_i, y_i)$ of $\mathcal{D}$ with their indices $i\in\{1,\ldots, n\}$.) Set $k:=1$ in the former case and $T:=\cup_{i=1}^k T_i$ in either case (so $T$ corresponds to the full training dataset).  Also select a test dataset for final model assessment.
